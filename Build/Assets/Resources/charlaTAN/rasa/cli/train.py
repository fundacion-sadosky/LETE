...
def run_training(args: argparse.Namespace, can_exit: bool = False) -> Optional[Text]:
    ...
    domain_path = os.path.join(DEFAULT_CHATBOTS_PATH, args.agent, DEFAULT_DOMAIN_PATH) if args.agent else args.domain
    config_path = os.path.join(DEFAULT_CHATBOTS_PATH, args.agent, DEFAULT_CONFIG_PATH) if args.agent else args.config
    data_path = [os.path.join(DEFAULT_CHATBOTS_PATH, args.agent, DEFAULT_DATA_PATH)] if args.agent else args.data
    output_path = os.path.join(DEFAULT_CHATBOTS_PATH, args.agent, DEFAULT_MODELS_PATH) if args.agent else args.out

    domain = rasa.cli.utils.get_validated_path(
        domain_path, "domain", DEFAULT_DOMAIN_PATH, none_is_valid=True
    )

    config = _get_valid_config(config_path, CONFIG_MANDATORY_KEYS)
