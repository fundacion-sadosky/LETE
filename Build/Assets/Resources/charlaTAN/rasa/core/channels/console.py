...
async def send_message_receive_block(
    ...
    url = ""
    if(receiver_id != ""):
        url = f"{server_url}/webhooks/rest/webhook?token={auth_token}&receiver={receiver_id}"
    else:
...
async def _send_message_receive_stream(
    ...
    url = ""
    if(receiver_id != ""):
        url = f"{server_url}/webhooks/rest/webhook?stream=true&token={auth_token}&receiver={receiver_id}"
    else:
    ...
    while not utils.is_limit_reached(num_messages, max_message_limit):
        ...
        if use_response_stream:
            bot_responses_stream = _send_message_receive_stream(
                server_url, auth_token, sender_id, text, request_timeout=request_timeout, receiver_id=receiver_id
            )
            previous_response = None
            ...
            bot_responses = await send_message_receive_block(
                server_url, auth_token, sender_id, text, receiver_id=receiver_id
            )
            previous_response = None
