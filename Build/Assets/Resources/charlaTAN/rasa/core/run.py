...
def serve_application(
    ...
    logger.info(f"Starting Charlatan server on {protocol}://{interface}:{port}")
...
async def load_agent_on_start(
    ...
    """
    Metodo eliminado porque no queremos cargar dicho agente
    para eso esta el CharlatanManager
    """
    ...
    logger.info("Charlatan server is up and running.")
...
def stop_prolog_processes_in_cache(cache: LRUCache):
    for key_name in cache.keys():
        if isinstance(cache[key_name], PrologThread):
            cache[key_name].halt_server()
        else:
            logger.warning("Se intenta finalizar algo que no es un proceso Prolog. Probablemente debido a que se "
                           "agrego algo a prolog_process_cache que no es un PrologThread")


def close_charlatan_resources():
    message_loop.close_execution()
    message_loop.join()
    from rasa.core.policies.prolog_policy_data.prolog_policy import prolog_process_cache
    stop_prolog_processes_in_cache(prolog_process_cache)
