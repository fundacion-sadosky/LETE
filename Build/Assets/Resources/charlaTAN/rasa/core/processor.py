...
class MessageProcessor:
    ...
    def __init__(
        ...
        self.agent_disable_timestamp = None
    ...
    def process_message_from_conversation(
        self, text, sender_id, conversation_id 
    ):
        ...
        conversation = charlatan_conversation_manager.get_conversation_by_id(conversation_id)
        
        if not conversation: 
            raise Exception(f"No existe conversacion cuyo id sea: {conversation_id}")
        conversation_data = conversation.to_dict()
        receiver_id = conversation_data["participant1"] if sender_id != conversation_data["participant1"] else conversation_data["participant2"]
        message_id = uuid.uuid4().hex
        self.save_message_in_conversation(
        ...
        if charlatan_manager.exists_agent(receiver_id):
            message_queue.put({
                "sender": sender_id,
    ...
    def save_message_in_conversation(
        self,
        message_id,
        conversation_id,
        sender,
    ...
    def deactivate_agent(self):
        self.agent_disable_timestamp = time.time()

    def activate_agent(self):
        self.agent_disable_timestamp = None

    def agent_activated(self) -> bool:
        if self.agent_disable_timestamp:
            if (time.time() - self.agent_disable_timestamp) <= (60*2):
                return False
        return True
    ...
    async def handle_message(
        ...
        tracker.agent_name = message.output_channel.get_agent_name()
        ...
        user_input = tracker.get_last_event_for(UserUttered)
        self.save_message_in_conversation(
                message_id=message.output_channel.get_message_id(),
                conversation_id=message.output_channel.get_conversation_id(),
                sender=message.sender_id,
                receiver=message.output_channel.get_agent_name(),
                text=user_input.text,
                interpretation=user_input.parse_data,
    ...
    async def log_message(
        ...
        """
        conversation_id = message.output_channel._conversation_id
