...
from conversation.search_criteria.message_criteria import MessageCriteria, MessageCriteriaTimestampGreater
...
def create_app(
    ...
    @app.post("/deactivate_agent/")
    async def deactivate_agent(request: Request) -> HTTPResponse:
        from CharlatanAccess import charlatan_manager
        agent_name_deactivate = request.args.get("agent_name", None)
        charlatan_manager.deactivate_agent(agent_name_deactivate)
        dict_to_export = {"code": "OK", "message": "disable agent."}
        return response.json(dict_to_export)
    
    @app.post("/activate_agent/")
    async def activate_agent(request: Request) -> HTTPResponse:
        from CharlatanAccess import charlatan_manager
        agent_name_activate = request.args.get("agent_name", None)
        charlatan_manager.activate_agent(agent_name_activate)
        dict_to_export = {"code": "OK", "message": "activated agent."}
        return response.json(dict_to_export)

    @app.get("/conversations/conversation-id/")
    async def get_conversation_id(request: Request) -> HTTPResponse:
        from CharlatanAccess import charlatan_conversation_manager
        from conversation.conversation_export import ListExport
        participant1 = request.args.get("participant1", None)
        participant2 = request.args.get("participant2", None)
        conversations = charlatan_conversation_manager.get_conversation_by_participants(participant1, participant2)
        dict_to_export = {"conversations": conversations}
        return response.json(dict_to_export)

    @app.get("/conversations")
    async def conversation_debug(request: Request) -> HTTPResponse:
        from CharlatanAccess import charlatan_conversation_manager
        from conversation.conversation_export import ListExport
        export_manager = ListExport(
            elements_to_export=charlatan_conversation_manager.get_all_conversations_id()
        )
        dict_to_export = {"conversations": export_manager.export()}
        return response.json(dict_to_export)
    
    @app.get("/conversations/<participant:string>")
    async def get_conversations_by_participant(request: Request, participant: Text) -> HTTPResponse:
        from CharlatanAccess import charlatan_conversation_manager
        from conversation.conversation_export import ListExport
        export_manager = ListExport(
            elements_to_export=charlatan_conversation_manager.get_conversations_by_participant(participant=participant)
        )
        dict_to_export = {"conversations": export_manager.export()}
        return response.json(dict_to_export)

    def get_conversations_tree_to_export(conversation_ids: List):
        from conversation.conversation_export import ListExport
        dict_to_export = {}
        export_manager = ListExport(
            elements_to_export=conversation_ids
        )
        dict_to_export["conversations"] = export_manager.export()
        return dict_to_export

    @app.get("/conversations/<conversation_id:string>/associated")
    async def conversation_history_of_agents_conversations(request: Request, conversation_id: Text) -> HTTPResponse:
        from CharlatanAccess import charlatan_conversation_manager
        conversations = charlatan_conversation_manager.get_children(conversation_id)
        conversations.insert(0, charlatan_conversation_manager.get_conversation_by_id(conversation_id))
        dict_to_export = get_conversations_tree_to_export(
            conversation_ids=conversations
        )
        return response.json(dict_to_export)

    def get_messages_from_conversation_list(conversation_id: Text, search_criteria: MessageCriteria):
        from CharlatanAccess import charlatan_message_history
        from conversation.conversation_export import ListExport
        dict_to_export = {}
        all_messages_in_conversation = charlatan_message_history.get_messages_of_conversation(conversation_id, search_criteria)
        export_manager = ListExport(
            elements_to_export=all_messages_in_conversation
        )
        dict_to_export["messages"] = export_manager.export()
        return dict_to_export

    @app.get("/conversations/<conversation_id:string>/messages")
    async def conversation_history(request: Request, conversation_id: Text) -> HTTPResponse:
        """
            Dado una Conversation-ID se retornan todos los mensajes asociados a esa conversacion.
            Utiliza el query-param llamado "timestamp" para aplicar un filtrado por el mismo. 
            Si el mensaje tiene un timestamp > al definido en el queryparam forma parte de la lista resultado
        """
        if not conversation_id:
            return ErrorResponse(
                HTTPStatus.INTERNAL_SERVER_ERROR,
                "ConversationError",
                "not conversation_id defined",
            )
        timestamp = request.args.get("timestamp", None)
        if timestamp:
            search_criteria = MessageCriteriaTimestampGreater(float(timestamp))
        else:
            search_criteria = None
        dict_to_export = get_messages_from_conversation_list(conversation_id, search_criteria)
        return response.json(dict_to_export)
